REATE OR REPLACE FUNCTION terrapop_extract_dense_cruts_data( IN sample_geog_level_id bigint, IN temp_path text)
  RETURNS TABLE(pixel_id bigint, geom geometry) AS
$BODY$
import numpy as np
...
def create_CRUTS_geometry(sgl_id, shape_location):
    ...
    FROM climate.cruts_grid_template c inner join country_boundary b on ST_Intersects(c.geom, b.geom) ''' % (sgl_id)
...
def CRUTSMetadata(sgl_id):
    '''This functin will get the bounding box for CRUTS data set based on the sgl.id '''
    query  = '''WITH country_boundary as
    (
    SELECT sgl.id as sample_geog_level_id, ST_Buffer(ST_Collect(ST_ConvexHull(bound.geog::geometry)),.5) as geom
    FROM sample_geog_levels sgl
    ...
    FROM climate.cruts_grid_template c inner join country_boundary b on ST_Within(c.geom, b.geom) ''' % (sgl_id)
    ...
    #Modified the return extents so they would 1 CRUTS pixel size larger.
    return [float(extent_w)-.5, float(extent_n)-.5, float(extent_e)+.5, float(extent_s)+.5]
...
def raster_densification(oldarray,densifier):
    ...
    #Repeat the array
        ...
        #denseArray = np.repeat(oldarray, densificationValue).tolist()            
...
def cruts_raster_densification(shapefilepath, cruts_array, geoTransform, geoproj, feature_count ):
    ...
    while found_geometries < feature_count:
        ...
        if len(unique_boundary_ids) < feature_count: 
            #Now increment                
            densifier *= 2
...
def create_cruts_data_table():
    try:    
        plpy.execute("Create TABLE cruts_dense (pixel_id bigint, x double precision, y double precision);")
    except plpy.SPIError, e:
        
        plpy.execute("DROP TABLE cruts_dense;")
        plpy.execute("Create TABLE cruts_dense (pixel_id bigint, x double precision, y double precision);")
    
def join_dense_array_to_cruts(m,y):

    if m:
        #This query requires both a month and a year
        query = '''With dense_cruts as
        (
        select pixel_id, ST_GeomFromText('POINT('||x||' '||y||')',4326) as geom
        from cruts_dense
        )
        select d.pixel_id, c.year, c.month, c.pre, d.geom
        from climate.cruts_322 c inner join dense_cruts d on d.pixel_id = c.pixel_id
        where c.month in (%s) and c.year = (%s) ''' % (m, y)
    else:
...
def return_dense_cruts_dense():

    query  = '''select pixel_id, ST_GeomFromText('POINT('||x||' '||y||')',4326) as geom from cruts_dense '''
...
def populate_CRUTS_table(outfilepath, pixel_data_array, x_start, y_start, x_res, y_res):
    outfilepath = r'/tmp/cruts_dense2.csv'
    ...
    with open(outfilepath, 'w') as f:
        while not it.finished:
            ...
            outtext =  '%s,%s,%s\n' % (int(pixel_id), centroid_x, centroid_y )

            f.write(outtext)
            it.iternext()
            
    copy_statement = "COPY %s FROM '%s' DELIMITER ',' " % ('cruts_dense', outfilepath)
...
CRUTS_BBox = CRUTSMetadata(sample_geog_level_id)
...
create_CRUTS_geometry(sample_geog_level_id, cruts_centroids_shape)
...
if len(CRUTS_GeoIDS) != num_features:
    ...
    create_cruts_data_table()
    populate_CRUTS_table(temp_txt_file_path, CRUTS_DENSE_PixelArray, start_x, start_y, x_resolution, y_resolution)
	...
	create_cruts_data_table()
	populate_CRUTS_table(temp_txt_file_path, CRUTS_PixelID_Array, start_x, start_y, x_resolution, y_resolution)
    
plpy.notice('Generating CRUTS Template data table')
data = return_dense_cruts_dense()
